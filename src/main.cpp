...
nlohmann::json merge_strats_json(std::vector<std::time_t> &all_dates, std::map<std::string, std::map<std::time_t, double>> strats_values){
    ...
    for (const auto& date : all_dates) {
        ...
        for (const auto& [strat_name, values_map] : strats_values) {
            double closest_value = get_closest_value(date, values_map);
...
nlohmann::json run_strategies(UserInputHandler &inputs)
{
    ...
    std::map<std::string, std::map<std::time_t, double>> strats_values;
    for (size_t strat_i = 0; strat_i < global_params.size(); ++strat_i)
    {
        ...
        std::map<std::time_t, double> strat_values = strat->run_strategy();
